...
import { getCurrentWeather, getForecast } from "../../services/apiClient";
...
const CustomDropdown = ({ label, value, options, onChange, icon: Icon }) => {
  ...
  return (
    <div className="flex-1 relative" ref={dropdownRef}>
      <label className="absolute -top-2.5 left-4 px-2 bg-white text-[9px] font-bold text-blue-600 uppercase tracking-widest z-10">
        {label}
      </label>
      <button
      ...
      >
        <Icon
          size={16}
          className={`${isOpen ? "text-blue-600" : "text-slate-400"} mr-3 transition-colors`}
        />
        <span className="text-sm font-bold text-slate-900">{value}</span>
        <ChevronDown
          size={16}
          className={`ml-auto text-slate-400 transition-transform duration-500 ${isOpen ? "rotate-180 text-blue-600" : ""}`}
        />
      </button>
...
export default function WeatherAnalytics() {
  ...
  const buildSeries = (forecastDaily, filter) => {
    const base = forecastDaily.map((item) => ({
      time: item.date?.slice(5) || item.date,
      temp: item.temp_avg ?? item.temp_max ?? item.temp_min ?? 0,
      rainfall: item.rainfall ?? 0,
    }));

    if (filter === "days") return base;
    ...
    if (filter === "weeks") {
      return Array.from({ length: 7 }, (_, i) => {
        ...
        return {
          time: src.time || `W${i + 1}`,
          temp: Math.round((src.temp + factor * 0.8) * 10) / 10,
          rainfall: Math.max(
            0,
            Math.round((src.rainfall + factor * 3) * 10) / 10,
          ),
        };
      });
    }
    ...
    if (filter === "months") {
      return Array.from({ length: 4 }, (_, i) => {
        const slice = base.slice(0, Math.max(1, Math.min(base.length, 2 + i)));
        const temp =
          slice.reduce((sum, item) => sum + item.temp, 0) / slice.length;
        const rainfall = slice.reduce((sum, item) => sum + item.rainfall, 0);
        return {
          time: `M${i + 1}`,
    ...
    return Array.from({ length: 4 }, (_, i) => ({
      time: String(new Date().getFullYear() - (3 - i)),
      temp: Math.round((base[0]?.temp || 24) + (i - 1.5) * 0.4),
      rainfall: Math.round(
        (base.reduce((sum, item) => sum + item.rainfall, 0) /
  ...
  useEffect(() => {
    ...
    const loadWeather = async () => {
      setIsSyncing(true);
      try {
        const coords = STATE_COORDS[selectedState] || STATE_COORDS.Punjab;
        const [forecastRes, currentRes] = await Promise.all([
          getForecast(coords.lat, coords.lng),
          getCurrentWeather(coords.lat, coords.lng),
        ]);

        const daily = Array.isArray(forecastRes?.data?.daily)
          ? forecastRes.data.daily
          : Array.isArray(forecastRes?.daily)
            ? forecastRes.daily
            : [];
        const baseDaily = daily.length
          ? daily
          : WEATHER_HISTORY.days.map((d, idx) => ({
              date: d.time || `D${idx + 1}`,
              temp_avg: d.temp,
              rainfall: d.rainfall,
            }));
        const series = buildSeries(baseDaily, timeFilter);

        if (mounted) {
          setLocalData(series);
          setCurrentWeather({
            temp:
              currentRes?.data?.temperature ??
              currentRes?.temperature ??
              series[0]?.temp ??
              24,
            humidity: currentRes?.data?.humidity ?? currentRes?.humidity ?? 62,
            windSpeed:
              currentRes?.data?.wind_speed ?? currentRes?.wind_speed ?? 14,
            condition:
              currentRes?.data?.description ??
              currentRes?.description ??
              "Partly Cloudy",
          });
          setSourceLabel(
            forecastRes?.data?.mock || currentRes?.data?.mock
              ? "mock"
              : "backend",
          );
        }
        ...
        if (mounted) {
          const fallback = buildSeries(
            WEATHER_HISTORY[timeFilter].map((item) => ({
              date: item.time,
              temp_avg: item.temp,
      ...
      return (
        <div className="h-full w-full flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] animate-pulse">
            Syncing {selectedRegion} Station...
          </p>
        </div>
      );
    ...
    <div className="space-y-5 animate-fadeIn pb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-2xl p-5 text-white shadow-lg relative overflow-hidden group hover:scale-[1.02] transition-all duration-500">
          ...
          <p className="text-[10px] font-bold text-blue-100 uppercase tracking-widest mb-3">
            Live Forecast · {sourceLabel}
          </p>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center gap-4 hover:bg-slate-50 transition-all duration-500 group">
          ...
          <div>
            ...
            <h3 className="text-2xl font-black text-slate-900 leading-none">
              {Math.round(currentWeather.temp || 24)}°C
            </h3>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center gap-4 hover:bg-slate-50 transition-all duration-500 group">
          ...
          <div>
            ...
            <h3 className="text-2xl font-black text-slate-900 leading-none">
              {Math.round(currentWeather.humidity || 62)}%
            </h3>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center gap-4 hover:bg-slate-50 transition-all duration-500 group">
          ...
          <div>
            ...
            <h3 className="text-2xl font-black text-slate-900 leading-none">
              {Math.round(currentWeather.windSpeed || 14)} km/h
            </h3>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-2xl p-6 md:p-8 border border-slate-200 shadow-xl relative overflow-hidden">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8 relative z-10">
          <div>
            <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter mb-1.5">
              Climatic Intelligence
            </h3>
            <p className="text-xs text-slate-500 font-medium">
              Station:{" "}
              <span className="text-blue-600 font-bold underline underline-offset-4 decoration-blue-600/20">
                AS-{(selectedRegion || "DEF").toUpperCase().slice(0, 3)}-026
              </span>{" "}
              · Active Data Streams
            </p>
          </div>
          ...
          <div className="flex items-center gap-5 p-5 rounded-2xl bg-slate-50 hover:bg-white transition-all border border-transparent hover:border-slate-200 hover:shadow-lg group">
            ...
            <div>
              ...
              <p className="text-base font-black text-slate-900">
                {localData.length > 0
                  ? [...localData].sort((a, b) => b.temp - a.temp)[0]?.temp
                  : "24"}
                °C
              </p>
          ...
          <div className="flex items-center gap-5 p-5 rounded-2xl bg-slate-50 hover:bg-white transition-all border border-transparent hover:border-slate-200 hover:shadow-lg group">
            ...
            <div>
              ...
              <p className="text-base font-black text-slate-900">
                {localData.length > 0
                  ? Math.round(
                      localData.reduce((acc, curr) => acc + curr.rainfall, 0) /
                        localData.length,
                    )
                  : "0"}{" "}
                mm
