  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      type: "ai",
      text: "AgriSense is ready. Ask me about crops, irrigation, pests, weather, or market prices.",
      time: "Sync: Active",
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await sendChatMessage(
        { message: messageText, language: "en" },
        token,
      );
      ...
      const aiMsg = {
        id: Date.now() + 1,
        type: "ai",
        text: "The backend chat service is unavailable right now. Please try again in a moment.",
        time: "Error",
      };
      setMessages((prev) => [...prev, aiMsg]);
    } finally {
      ...
      >
        <div className="max-w-4xl mx-auto space-y-8">
          {messages.map((msg) => (
            ...
            >
              <div
                className={`flex gap-4 max-w-[85%] ${msg.type === "user" ? "flex-row-reverse" : ""}`}
              >
                <div
                  className={`w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center shadow-sm ${
                    msg.type === "ai"
                      ? "bg-white text-green-600 border border-slate-200"
                ...
                <div
                  className={`space-y-1 ${msg.type === "user" ? "items-end" : ""}`}
                >
                  <div
                    className={`p-5 rounded-3xl text-sm font-black leading-relaxed shadow-xl ${
                      msg.type === "ai"
