...
const PEST_LIBRARY = [
  {
    ...
    severity: "High",
    symptoms:
      "Small, orange-brown pustules on leaves, reducing photosynthesis.",
    solution:
      "Apply Propiconazole or Tebuconazole fungicide at first sign of infection. Ensure proper drainage.",
  ...
  },
  {
    ...
    solution:
      "Use resistant varieties. Apply Tricyclazole 75% WP or Azoxystrobin.",
    pesticide: "Baan Fungicide",
    pesticideImg:
      "https://images.unsplash.com/photo-1628352081506-83c43123ed6d?q=80&w=1000&auto=format&fit=crop",
    image:
      "https://images.unsplash.com/photo-1536630596251-b12ba0d9f7d4?q=80&w=1000&auto=format&fit=crop",
  },
  {
    id: 3,
    name: "Downy Mildew",
    ...
    solution:
      "Seed treatment with Metalaxyl. Spraying with Mancozeb if symptoms persist.",
    pesticide: "Ridomil Gold",
    pesticideImg:
      "https://images.unsplash.com/photo-1622560480605-d83c853bc5c3?q=80&w=1000&auto=format&fit=crop",
    image:
      "https://images.unsplash.com/photo-1551009175-8a68da93d5f9?q=80&w=1000&auto=format&fit=crop",
  },
  {
    id: 4,
    name: "Bollworm",
    ...
    solution:
      "Integrated Pest Management (IPM). Use Bt Cotton varieties. Spray Emamectin Benzoate.",
    pesticide: "Delegate Insecticide",
    pesticideImg:
      "https://images.unsplash.com/photo-1605173574183-ef9849814763?q=80&w=1000&auto=format&fit=crop",
    image:
      "https://images.unsplash.com/photo-1594904351111-a072f80b1a71?q=80&w=1000&auto=format&fit=crop",
  },
];
...
export default function PestDetection({ location = "Punjab" }) {
  ...
  const analyzeImage = async (file) => {
    ...
    try {
      const response = await detectPest(file);
      const payload = response?.data?.prediction || response?.data || response;
      setAnalysisResult({
        id: Date.now(),
        name: payload?.disease || payload?.class || "Unknown Disease",
        crop: payload?.crop || "Unknown",
        severity: payload?.severity
          ? payload.severity[0].toUpperCase() + payload.severity.slice(1)
          : "Unknown",
        symptoms:
          payload?.symptoms ||
          payload?.treatment ||
          "Backend classification completed.",
        solution:
          payload?.treatment ||
          payload?.prevention ||
          "Consult agronomy guidance for next steps.",
        pesticide:
          payload?.disease || payload?.class || "Recommended Treatment",
        pesticideImg: PEST_LIBRARY[0].pesticideImg,
        image: selectedImage,
      });
    } catch {
      const randomResult =
        PEST_LIBRARY[Math.floor(Math.random() * PEST_LIBRARY.length)];
  ...
  useEffect(() => {
    let mounted = true;
    const loadCommonPests = async () => {
      try {
        const crop =
          filterCrop === "All Crops" ? "tomato" : filterCrop.toLowerCase();
        const response = await getCommonPests(crop);
        const payload = response?.data || response;
        const entries =
          payload && !Array.isArray(payload)
            ? Object.entries(payload).map(([key, value]) => ({ key, value }))
            : [];
        if (mounted) setCommonPests(entries);
      } catch {
        if (mounted) setCommonPests([]);
      }
    };
    loadCommonPests();
    return () => {
  ...
  const filteredLibrary = PEST_LIBRARY.filter(
    (item) =>
      ...
      <section className="bg-white/80 backdrop-blur-2xl rounded-[3rem] p-8 md:p-12 border border-slate-200 shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row gap-12 relative z-10">
          <div className="lg:w-1/2 space-y-8">
            <div>
              ...
              <p className="text-slate-500 font-medium leading-relaxed">
                Upload a photo of your crop or use your camera for real-time
                analysis. Our neural network detects over 250+ diseases with 94%
                accuracy.
              </p>
            </div>
          ...
          <div className="lg:w-1/2">
            <div
              className={`relative h-80 rounded-[2.5rem] border-2 border-dashed transition-all duration-500 flex flex-col items-center justify-center overflow-hidden ${selectedImage ? "border-green-600" : "border-slate-200 bg-slate-50/50 hover:bg-slate-50"}`}
            >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-400 font-medium mt-1">
                        Tap to camera or drag image here
                      </p>
                    </div>
                  ...
                  >
                    ...
                    {isAnalyzing && (
                      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm flex flex-col items-center justify-center text-white">
                        ...
                        <p className="text-xs font-black uppercase tracking-[0.3em] animate-pulse">
                          Neural Mapping...
                        </p>
                      </div>
                    )}
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                ...
                <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-lg flex flex-col items-center text-center">
                  <img
                    src={analysisResult.pesticideImg}
                    alt={analysisResult.pesticide}
                    className="w-24 h-24 object-cover rounded-2xl mb-6 shadow-md"
                  />
      ...
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 px-4">
          <div>
            ...
            <p className="text-sm text-slate-400 font-medium">
              Browse verified solutions for common regional pests
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {filteredLibrary.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 group"
            >
