    ...
    options: [
      {
        label: "North India",
        icon: "🏔️",
        description: "Cold winters, hot summers.",
      },
      {
        label: "South India",
        icon: "🌴",
        description: "Warm and humid all year.",
      },
      { label: "West India", icon: "🏜️", description: "Dry and very hot." },
      {
        label: "East India",
        icon: "🎋",
        description: "Lots of rain and floods.",
      },
      {
        label: "Central India",
        icon: "🌋",
        description: "Moderate rain, central plains.",
      },
    ],
  },
  {
    ...
    options: [
      {
        label: "Soft & Crumbly",
        icon: "🌱",
        description: "Good dark soil, easy to dig.",
      },
      {
        label: "Black & Sticky",
        icon: "🌑",
        description: "Cracks when dry, holds water.",
      },
      {
        label: "Sandy / Red",
        icon: "🏖️",
        description: "Water drains very fast.",
      },
      {
        label: "Heavy Clay",
        icon: "🏺",
        description: "Sticks to hands when wet.",
      },
    ],
  },
  {
    ...
    options: [
      {
        label: "Wheat or Rice",
        icon: "🌾",
        description: "Regular food grains.",
      },
      {
        label: "Dal / Pulses",
        icon: "🥜",
        description: "Chana, Moong, Toor etc.",
      },
      {
        label: "Vegetables",
        icon: "🥦",
        description: "Tomato, Onion, Potato etc.",
      },
      {
        label: "Nothing (Empty)",
        icon: "🏜️",
  ...
  },
  {
    ...
    options: [
      { label: "Very Good", icon: "📈", description: "Got a lot of crop." },
      {
        label: "Normal / Okay",
        icon: "📊",
        description: "Just regular amount.",
      },
      {
        label: "Poor / Failed",
        icon: "📉",
        description: "Crop got ruined or very less.",
      },
    ],
  },
  {
    ...
    options: [
      {
        ...
        icon: "🚰",
        description: "Can pump water anytime.",
      },
      {
        label: "Canal Water",
        icon: "🌊",
        description: "Comes when government releases.",
      },
      {
        label: "Only Rain",
        icon: "🌧️",
        description: "Depend on monsoon clouds.",
      },
      {
        label: "Drip Pipes",
        icon: "💧",
        description: "Small pipes giving water to roots.",
      },
    ],
  },
  {
    ...
    options: [
      {
        label: "I can spend well",
        icon: "💎",
        description: "Want to grow costly crops.",
      },
      { label: "Medium", icon: "💳", description: "Normal expenses only." },
      {
        label: "Very Little",
        icon: "🪙",
        description: "Need very low-cost crops.",
      },
    ],
  },
];
...
const REGION_MAP = {
  ...
  "West India": { season: "zaid", temperature: 35, humidity: 42, rainfall: 20 },
  "East India": {
    season: "kharif",
    temperature: 29,
    humidity: 72,
...
const BACKEND_TASKS = {
  wheat: [
    ...
    "Monitor rust and irrigation",
  ],
  rice: [
    "Prepare nursery and transplant on time",
    "Keep water level steady",
    "Watch for leaf blast",
  ],
  maize: [
    "Ensure proper spacing",
    "Apply nitrogen in stages",
    "Scout for borers",
  ],
  potato: [
  ...
  tomato: [
    "Install staking or trellis",
    "Mulch to keep soil moist",
    "Inspect for blight early",
  ],
  onion: [
...
const pickTasks = (cropName) =>
  BACKEND_TASKS[cropName?.toLowerCase?.()] || [
    "Follow the agronomist guidance",
    "Use recommended seed and fertilizer",
    "Track weather before sowing",
  ];

const extractRecommendation = (response) => {
  const payload = response?.data?.prediction || response?.data || response;
  const recommendations =
    ...
    payload?.top3 ||
    [];
  const topCrop = Array.isArray(recommendations)
    ? recommendations[0]?.crop || recommendations[0]?.name || recommendations[0]
    : payload?.crop ||
      payload?.recommended_crop ||
      payload?.best_crop ||
      "Turmeric";
  const cropName =
    typeof topCrop === "string"
      ? topCrop
      : topCrop?.crop || topCrop?.name || "Turmeric";
  const confidence =
    payload?.confidence ||
    payload?.score ||
  ...
  return {
    ...
    reason:
      ...
      payload?.reason ||
      payload?.advice ||
      "This recommendation is generated from the backend crop model.",
    tasks: pickTasks(cropName),
    profitPotential: payload?.profitPotential || "Backend Assisted",
    marketPrice:
      payload?.marketPrice ||
      payload?.market_price ||
      "Check MSP / market panel",
  };
};
...
export default function CropRecommendation() {
  ...
  const startAnalysis = async (finalAnswers) => {
    ...
    try {
      const region =
        REGION_MAP[finalAnswers.region] || REGION_MAP["Central India"];
      const soil_condition = SOIL_MAP[finalAnswers.soil] || "loamy";
      const body = {
        soil_condition,
      ...
      const response = await recommendCrop(body);
      setRecommendation(extractRecommendation(response));
    } catch {
      setRecommendation({
        crop: "Turmeric (Haldi)",
        confidence: 88,
        reason:
          "The backend recommendation service is unavailable right now, so this is a safe fallback crop.",
        tasks: [
            ...
            <div className="space-y-12">
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 font-medium">
                      Deep analysis of your farm's history and potential.
                    </p>
                  </div>
                </div>
              ...
              <div className="flex gap-1.5">
                {QUESTIONS.map((_, i) => (
                  <div
                    key={i}
                    className={`h-1.5 flex-1 rounded-full transition-all duration-700 ease-out ${i <= step ? "bg-green-600 shadow-sm" : "bg-slate-100"}`}
                  ></div>
                ))}
              </div>
              ...
              <div className="space-y-10">
                <div className="space-y-3">
                  ...
                  <h3 className="text-4xl font-black text-slate-900 leading-[1.1]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {QUESTIONS[step].options.map((opt) => (
                    ...
                    >
                      <div className="text-3xl bg-slate-50 w-16 h-16 rounded-xl flex items-center justify-center shadow-sm group-hover:scale-110 group-hover:bg-green-600/5 transition-all duration-500">
                        {opt.icon}
                      </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col gap-2 text-slate-400 text-sm font-medium">
                  <p className="flex items-center justify-center gap-2">
                    <Globe size={14} className="text-blue-600" /> Analyzing
                    climatic variables
                  </p>
                  <p className="flex items-center justify-center gap-2">
                    <RotateCcw size={14} className="text-amber-600" /> Factoring
                    rotation history
                  </p>
                  <p className="flex items-center justify-center gap-2">
                    <TrendingUp size={14} className="text-green-600" />{" "}
                    Benchmarking yield trends
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-12 animate-fadeIn">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 font-medium">
                      Optimized for maximum profit and soil health.
                    </p>
                  </div>
                </div>
                <button
                  onClick={reset}
                  className="flex items-center gap-2 px-6 py-3 bg-slate-50 hover:bg-white rounded-2xl text-xs font-black text-slate-400 transition-all border border-slate-200 shadow-sm uppercase tracking-widest"
                >
                  ...
                  <div className="lg:col-span-7 space-y-8">
                    <div>
                      <div className="flex items-center gap-3 mb-4">
                        <span className="text-[10px] font-black text-green-600 bg-green-50 px-3 py-1 rounded-full border border-green-100 uppercase tracking-[0.2em]">
                          High Value Target
                        </span>
                      ...
                      <h3 className="text-6xl font-black text-slate-900 tracking-tight leading-none">
                        {recommendation.crop}
                      </h3>
                    </div>
                    <p className="text-slate-500 text-lg leading-relaxed italic border-l-4 border-green-600 pl-6 font-medium">
                      ...
                      <div className="space-y-4">
                        {recommendation.tasks.map((task, i) => (
                          <div key={i} className="flex items-start gap-4 group">
                            <div className="w-6 h-6 rounded-full bg-green-50 text-green-600 flex items-center justify-center text-[10px] font-black border border-green-100 group-hover:bg-green-600 group-hover:text-white transition-all flex-shrink-0 mt-0.5">
                              {i + 1}
                            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="bg-amber-50 p-5 rounded-2xl border border-amber-100 shadow-sm">
                  ...
                  <p className="text-sm font-black text-slate-700 text-truncate">
                    {answers.soil || "Analyzing"}
                  </p>
                </div>
