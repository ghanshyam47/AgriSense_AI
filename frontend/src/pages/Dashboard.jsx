      ...
      >
        ...
        <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-1.5">
            ...
            <input
              type="text"
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium focus:bg-white focus:border-green-500 focus:ring-2 focus:ring-green-500/20 outline-none transition-all placeholder:text-slate-400"
        ...
        <div className="p-5 border-t border-slate-100 flex justify-end gap-3 bg-slate-50/80">
          ...
          <button
            onClick={onClose}
            className="px-5 py-2.5 text-sm font-bold text-white bg-green-600 hover:bg-green-700 rounded-xl transition-all shadow-lg shadow-green-200 hover:-translate-y-0.5"
          >
            Save Farm Details
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar bg-slate-50/50 flex-1">
          <div className="space-y-4">
            {farms.map((farm) => (
              <div
                key={farm.id}
                className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 group hover:border-blue-300 transition-all hover:shadow-md"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-500 mt-1 uppercase tracking-wider">
                      {farm.area} • {farm.crop} • {farm.soil}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button className="px-4 py-2 text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-600 hover:text-white rounded-xl transition-all border border-blue-100">
                    Edit
                  </button>
                  <button className="px-4 py-2 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-600 hover:text-white rounded-xl transition-all border border-red-100">
                    Delete
                  </button>
                </div>
              </div>
            ...
            <div className="border-2 border-dashed border-slate-200 rounded-xl p-8 flex flex-col items-center justify-center text-center bg-white hover:bg-slate-50 transition-colors">
              ...
              <h4 className="text-sm font-black text-slate-700 mb-1">
                Need to track another field?
              </h4>
              <p className="text-xs font-medium text-slate-500 mb-5">
                Add all your farm properties to get AI insights.
              </p>
              <button
                onClick={() => {
                  onClose();
...
export default function Dashboard({ language, setLanguage }) {
  ...
  useEffect(() => {
    ...
    const loadAlerts = async () => {
      ...
      try {
        ...
        const response = await getAlerts(token, { limit: 20 });
        const rows = Array.isArray(response?.data)
          ? response.data
          : Array.isArray(response)
            ? response
            : [];
        const normalized = rows.map((alert) => ({
          ...
          action: alert.actionable || "View Details",
        }));
        if (mounted && normalized.length > 0) setActiveAlerts(normalized);
      } catch (_err) {
        if (mounted) setActiveAlerts(SMART_ALERTS);
      } finally {
  ...
  const renderContent = () => {
    switch (activeNav) {
      case "overview":
        return <Overview setIsChatOpen={setIsChatOpen} />;
      case "market":
        return <MarketMSP />;
      case "irrigation":
        return (
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(135deg, #f9fafb 0%, #ecfdf5 40%, #f9fafb 100%)",
      }}
    >
      {/* ─── SUBTLE GRID PATTERN ─── */}
      <div
        ...
        <header className="h-16 bg-white/90 backdrop-blur-xl border-b border-slate-200 shadow-sm flex items-center justify-between px-6 md:px-8 flex-shrink-0 z-20 relative transition-all">
          <div className="flex items-center gap-5">
            <div className="flex items-center space-x-2.5">
              <div className="w-9 h-9 bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-inner flex items-center justify-center p-0.5">
                <img
                  src="/logo.jpeg"
                  alt="AgriSense"
                  className="w-full h-full rounded-lg object-contain brightness-110 bg-white"
                />
              </div>
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-[10px] text-green-600 font-bold uppercase tracking-widest leading-tight">
                {t.agriIntel}
              </p>
            </div>
          </div>
            ...
            <div className="relative" ref={languageRef}>
              ...
              >
                <Globe
                  size={16}
                  className={
                    showLanguageDropdown ? "text-white" : "text-green-600"
                  }
                />
                ...
                <div className="absolute right-0 mt-3 w-80 md:w-96 bg-white backdrop-blur-xl rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-slate-200 overflow-hidden z-[100] animate-fadeInUp">
                  <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    ...
                    <span className="bg-green-600 text-white px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider">
                      {isAlertsLoading ? "..." : `${activeAlerts.length} New`}
                    </span>
                  </div>
                          ...
                          <div className="flex items-start gap-4">
                            <div
                              className={`w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center ${
                                alert.type === "weather" ||
                                alert.type === "weather-irrigation"
                                  ? "bg-blue-100 text-blue-600"
                                  : alert.type === "pest" ||
                                      alert.type === "pest-weather"
                            ...
                            >
                              ...
                              {(alert.type === "market" ||
                                alert.type === "market-trend") && (
                                <TrendingUp size={18} />
                              )}
                            </div>
                            <div className="flex-1">
                              <div className="flex items-center justify-between mb-1">
                                <h4 className="text-sm font-bold text-slate-900 group-hover:text-green-600 transition-colors">
                                  {alert.title}
                                </h4>
                                <span
              ...
              {activeNav === "overview" &&
                ...
                activeAlerts.map((alert) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <h4 className="text-xs font-black text-slate-900 pr-6">
                        {alert.title}
                      </h4>
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Label under icon */}
                  <span
                    className={`text-[8px] font-black uppercase tracking-[0.1em] transition-all ${active ? getIconColor() : "text-slate-400 group-hover:text-slate-600"}`}
                  >
                  ...
                  {active && (
                    <div
                      className={`absolute -bottom-0.5 w-1 h-1 rounded-full ${getIconColor()} bg-current`}
                    />
                  )}
                </motion.button>
              );
      ...
      <AnimatePresence>
        ...
        {showManageFarmsModal && (
          <ManageFarmsModal
            isOpen={showManageFarmsModal}
            onClose={() => setShowManageFarmsModal(false)}
            onAdd={() => setShowAddFarmModal(true)}
          />
        )}
      </AnimatePresence>
    </div>
